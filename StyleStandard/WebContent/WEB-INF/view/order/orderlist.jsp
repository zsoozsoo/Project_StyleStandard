        ...
        <div class="container">
            ...
            <div class="table-responsive">
              <table class="table" style="text-align: center;">
                ...
                <tr> 
                <td>1</td>
                <td>${orderlist[0].order_code}</td>
 <td><%= request.getParameter("name") %></td>
             <% session.setAttribute("paymentName", request.getParameter("name")); %>
             <td><%= request.getParameter("curTime")%></td>
             <% session.setAttribute("paymentDate", request.getParameter("curTime")); %>
             <% session.setAttribute("returnDate", request.getParameter("returnTime")); %>
             <td><%= request.getParameter("amount") %>원</td>
             <td>배송중</td>
             <td><p id="invoiceNumberText">385806775030<p><!-- <select id="deliveryList" name="tekbeCompnayList"></select> --><button id="myButton1">택배 조회하기 </button>
...
$(document).ready(function(){
    var myKey = "X8DAVTD9PQcLGOeAWW7qIA"; // sweet tracker에서 발급받은 자신의 키 넣는다.
        ...
        $.ajax({
            ...
            success:function(data){
                    ...
                    $("#deliveryList").html(myData);
            }
        });
        // 배송정보와 배송추적 tracking-api
        $("#myButton1").click(function() {
            var t_code = /* $('#deliveryList option:selected').attr('value'); */"04";
            var t_invoice = $('#invoiceNumberText').text();
            $.ajax({
                type:"GET",
                dataType : "json",
