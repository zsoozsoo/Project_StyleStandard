...
public class CustomerDao {
	...
	//밑에 customerSelectById랑 중복되므로 삭제
	 ...
	 pstm = conn.prepareStatement(query);
	  ...
	  if(rset.getString("user_id").equals("admin")) {
		  customer.setAuthority_cd("관리자");
		  }else{
	...
	public int updateCustomer(Connection conn, Customer customer) {
		...
		try {
			String query = "update st_customer set authority_cd = ?,user_pw = ?,name=?,email=?,sex=? where user_id = ?";
			...
			pstm.setString(4, customer.getEmail());
			pstm.setString(5, customer.getSex());
			pstm.setString(6, customer.getUser_id());
